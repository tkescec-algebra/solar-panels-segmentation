...
def create_dataloaders(val_dir, test_dir, batch_size=8, num_workers=4):
    ...
    # Dataset creation
    val_dataset = DATASET(data_dir=val_dir, transforms=val_transform, grayscale=GRAYSCALE, channels=IN_CHANNELS)
    test_dataset = DATASET(data_dir=test_dir, transforms=test_transform, grayscale=GRAYSCALE, channels=IN_CHANNELS)
...
def main_test():
    # Dataset paths
    val_images_dir = "../datasets/dataset/valid"
    test_images_dir = "../datasets/dataset/test"
    ...
    # DataLoaders
    dataloaders = create_dataloaders(val_images_dir, test_images_dir, batch_size=batch_size, num_workers=num_workers)
    ...
    # Model loading
    num_classes = 1
    model = get_model(num_classes=num_classes, model_name='fcn_resnet50', in_channels=IN_CHANNELS)
    model_path = "models/FCN-ResNet50_02_grayscale.pth"  # Putanja do treniranog modela
